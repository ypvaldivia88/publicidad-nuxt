    ...
    <b-col>
      ...
      <b-table striped hover :items="list" :fields="fields">
        <template #cell(nombre)="data">
          <NuxtLink :to="`/publicities/${data.item.id}/edit`">{{data.item.nombre}}</NuxtLink>
        </template>
        <template #cell(actions)="data">
          <b-button-group>
            <b-button variant="info" @click="$router.push(`/publicities/${data.item.id}/edit`)" >Editar</b-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      destroy() {
        this.$store.dispatch('publicities/delete', {id: this.id}).then(() => this.$store.dispatch('publicities/get'))
      }
    }
  }
</script>